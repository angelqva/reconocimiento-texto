...
def lenet(input_shape: Tuple[int, ...], output_shape: Tuple[int, ...]) -> Model:
    """Crea un modelo lenet
    ENTRADA => CONV => RELU => CONV => RELU => GRUPO => ABANDONO => Aplanar => FC => RELU => ABANDONO => FC
    Argumentos:
    input_shape : shape del tensor de entrada
    output_shape : shape del tensor de salida
    ...
    if len(input_shape) < 3:
        model.add(Lambda(lambda x: K.expand_dims(
            x, -1), input_shape=input_shape))
